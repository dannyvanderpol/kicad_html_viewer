...
)`;var d=class{constructor(e,t){this.debug=h.OFF,this.canvas=e,this.filename=t,this.content=null,this.sheet=null,this.colors=null,this.pageSize=m.A4,this.viewportTransform={x:0,y:0,scale:1},this.fitScaleX=1,this.fitScaleY=1,this.scaleSpeed=1/250,this.minScale=1/50,this.previousX=0,this.previousY=0,this.isMoving=!1,this.canvas.addEventListener("mousedown",this._onMouseDown.bind(this)),this.canvas.addEventListener("mouseup",this._onMouseUp.bind(this)),this.canvas.addEventListener("mousemove",this._onMouseMove.bind(this)),this.canvas.addEventListener("wheel",this._onMouseWheel.bind(this))}async initialize(){this.debug&h.VIEWER&&console.log("Viewer: drawing KiCad content");let e=new p(this.filename,this.debug&h.READER);this.content=await e.loadFile(),this.debug&h.GENERAL&&console.log("Load default sheet"),e=new p("pageLayout",this.debug&h.READER),this.sheet=e.parseFile(x),this.colors=new u(this.content.type,this.debug&h.COLORS),this.pageSize=m[this.content.paper],this.pageSize||(this.debug&h.GENERAL&&console.warn("Unknown page size:",this.content.paper),this.pageSize=m.A4),this.fitScaleX=this.canvas.width/this.pageSize.width,this.fitScaleY=this.canvas.height/this.pageSize.height,this.viewportTransform.scale=Math.min(this.fitScaleX,this.fitScaleY)*.98,this.viewportTransform.x=(this.canvas.width-this.pageSize.width*this.viewportTransform.scale)/2,this.viewportTransform.y=(this.canvas.height-this.pageSize.height*this.viewportTransform.scale)/2,this._render()}_render(){let e=this.canvas.getContext("2d");e.setTransform(1,0,0,1,0,0),e.clearRect(0,0,this.canvas.width,this.canvas.height);let t=this.viewportTransform.scale/this.fitScaleX,r=this.viewportTransform.scale/this.fitScaleY,i=this.canvas.width*.95,s=-this.canvas.width*.95,a=this.canvas.height*.95,n=-this.canvas.height*.95;this.viewportTransform.x=this.viewportTransform.x>i?i:this.viewportTransform.x,this.viewportTransform.x=this.viewportTransform.x/t<s?s*t:this.viewportTransform.x,this.viewportTransform.y=this.viewportTransform.y>a?a:this.viewportTransform.y,this.viewportTransform.y=this.viewportTransform.y/r<n?n*r:this.viewportTransform.y,e.setTransform(this.viewportTransform.scale,0,0,this.viewportTransform.scale,this.viewportTransform.x,this.viewportTransform.y);let o=new _(e,this.viewportTransform.scale,this.colors,this.debug&h.DRAWER);o.drawPageOutline(this.pageSize),o.drawContent(this.sheet)}_onMouseDown(e){this.debug&h.EVENTS&&console.log("Mouse down:",e),this.previousX=e.clientX,this.previousY=e.clientY,this.isMoving=!0}_onMouseUp(e){this.debug&h.EVENTS&&console.log("Mouse up:",e),this.isMoving=!1}_onMouseMove(e){if(!this.isMoving)return;this.debug&h.EVENTS&&console.log("Mouse move:",e);let t=e.clientX,r=e.clientY;this.viewportTransform.x+=t-this.previousX,this.viewportTransform.y+=r-this.previousY,this.previousX=t,this.previousY=r,this._render()}_onMouseWheel(e){this.debug&h.EVENTS&&console.log("Mouse wheel:",e),e.preventDefault();let t=this.viewportTransform.x,r=this.viewportTransform.y,i=e.clientX,s=e.clientY,a=this.viewportTransform.scale;this.viewportTransform.scale+=e.deltaY*-this.scaleSpeed,this.viewportTransform.scale<this.minScale&&(this.viewportTransform.scale=this.minScale),this.viewportTransform.x=i-(i-t)*(this.viewportTransform.scale/a),this.viewportTransform.y=s-(s-r)*(this.viewportTransform.scale/a),this._render()}},m={A0:{width:1189,height:841},A1:{width:841,height:594},A2:{width:594,height:420},A3:{width:420,height:297},A4:{width:297,height:210}},h={OFF:0,VIEWER:1,READER:2,DRAWER:4,COLORS:8,ALL:255};document.addEventListener("DOMContentLoaded",async function(){w();for(let g of document.querySelectorAll('canvas[type="application/kicad"]')){let e=g.getAttribute("src");e&&await new d(g,e).initialize()}});})();
